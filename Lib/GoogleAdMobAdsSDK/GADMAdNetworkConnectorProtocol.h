...
//
//  Ad Network Adapters interact with the Mediation SDK using an object
//  that implements the GADMAdNetworkConnector protocol. The Connector object
//  can be used to obtain necessary information for ad requests, and to call
//  back to the Mediation SDK on ad request returns and user interactions.
//
...
// Single string publisher ID for the Adapter, supplied by the Mediation server
// and set by the user in mediation.admob.com . For SDKs requiring
// more than one string, use the -credentials method.
- (NSString *)publisherId;

// For SDKs requiring more than one string to identify the publisher, use
// this method to obtain credentials.
...
// When you need to show a landing page or any other modal
// view, such as when a user clicks or when your Ads SDK needs to show an
// interstitial, use this method to obtain a UIViewController that you can use
// to show your modal view. Call the -presentModalViewController:animated:
// method of the returned UIViewController .
- (UIViewController *)viewControllerForPresentingModalView;

// The end user's gender set by the App in GADRequest. If none specified,
...
// and userLocationAccuracyInMeters will all return 0. You may use these to
// construct CLLocation objects. According to Apple's guidelines, app publishers
// should not to use Core Location just for advertising. By not requiring
// CLLocation, publishers will not have to include the Core Location framework
...
// Keywords describing the current activity of the user such as @"Sport Scores",
// set by the App. If none, returns nil.
...
// view: pass along the ad network's own view.
...
// error: pass along any error object sent from the ad network's SDK, nil
// if not available.
- (void)adapter:(id<GADMAdNetworkAdapter>)adapter
    didFailAd:(NSError *)error;

// Adapter calls this when it receives an interstitial ad.
// interstitial: pass along ad network's interstitial object, or nil if not
// available.
- (void)adapter:(id<GADMAdNetworkAdapter>)adapter
...
- (void)adapter:(id<GADMAdNetworkAdapter>)callingAdapter
    clickDidOccurInBanner:(UIView *)view;

// Adapter should call as many of these as possible, when the user taps an ad
// and a full-screen modal appears.
...
// Adapter should call as many of these as possible, during the lifecycle
// of the returned interstitial.
...
// Adapter should call this method, when a user action will result in App
// switching
